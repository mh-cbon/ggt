...
func (t RestClient) Login(postLogin string, postPassword string) (jsonResBody *User, login *http.Cookie, err error) {
	sReqURL := "/login"
	...
	form := url.Values{}
	form.Add("login", postLogin)
	form.Add("password", postPassword)
...
func (t RestClient) Logout(cookieLogin string) (login *http.Cookie, err error) {
	sReqURL := "/logout"
	reqURL, URLerr := url.ParseRequestURI(sReqURL)
	if URLerr != nil {
...
func (t RestClient) Create(postLogin string, postPassword string) (jsonResBody *User, err error) {
	sReqURL := "/create"
	reqURL, URLerr := url.ParseRequestURI(sReqURL)
	if URLerr != nil {
		return nil, errors.New("todo")
