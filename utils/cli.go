// Package utils for the generators.
package utils
...
// GetPkgToLoad return the package path on cwd
...
// FindOutPkg identify the package name for given todo arg
func FindOutPkg(todo TransformArg) string {
	if todo.ToPkgPath != "" {
	...
	if todo.ToPkgPath == "" {
		prog := astutil.GetProgramFast(GetPkgToLoad())
...
func (t TransformArg) String() string {
	ret := ""
	ret += fmt.Sprintln("todo from pkg id:", t.FromPkgID)
...
func (t TransformArgs) Parse(args []string) (TransformArgs, error) {
	for _, arg := range args {
		...
		if strings.Index(y[0], "/") > -1 {
			...
			c.FromPkgID = strings.Split(y[0], "/")[0]
		}
